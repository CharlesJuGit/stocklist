...
// history 由轉檔器滾動維護：[{date, long:[{id,name,mkt}]}]，新→舊最多 3 筆。
// 「連續 ×N」＝含本週的**連續**出現次數，**斷週即重算**（中間有一週未入選就停止累計）。
function _streakMap(cur, hist) {
  const out = {};
  const sets = hist.map(h => new Set((h.long || []).map(x => x.id)));   // [上週, 2週前, 3週前]
  for (const s of cur) {
    let n = 1;
    for (const st of sets) { if (st.has(s.id)) n++; else break; }       // 斷一週就停
    out[s.id] = n;
  }
  return out;
}
// 折疊區塊：只列「前期入選但本週未續選」；同股多週只列最近一次；history 空則整塊不渲染
function renderHistoryBlock(cur, hist) {
  const box = document.getElementById('long-history');
  if (!box) return;
  box.innerHTML = '';
  if (!hist.length) return;                       // 第一週上線時 history 為空＝正常
  const curIds = new Set(cur.map(s => s.id));
  const seen = new Set();
  const LABEL = ['上週', '2週前', '3週前'];
  const groups = [];
  hist.forEach((h, i) => {
    const rows = (h.long || []).filter(s => !curIds.has(s.id) && !seen.has(s.id));
    rows.forEach(s => seen.add(s.id));            // 同股多週只列最近一次
    if (rows.length) groups.push({ label: LABEL[i] || `${i + 1}週前`, date: (h.date || '').slice(5), rows });
  });
  ...
  const body = groups.map(g => `
    <div class="mt-2">
      <div class="text-xs mb-1">
        <span class="px-1.5 py-0.5 rounded bg-blue-900/60 text-blue-300">${g.label}${g.date ? `（${g.date}）` : ''}</span>
        <span class="text-gray-500 ml-1">${g.rows.length} 支</span>
      </div>
      <div class="space-y-2" id="hist-grp-${g.label}"></div>
    </div>`).join('');
  box.innerHTML = `
    <details class="bg-gray-900/60 border border-gray-800 rounded-lg px-3 py-2">
      <summary class="cursor-pointer text-sm text-gray-400 select-none">前三週入選・本週未續選（${total} 支）</summary>
      <div class="text-[11px] text-gray-600 mt-1">曾在前三週的多頭清單、本週未再入選；點列可開個股彈窗</div>
      ${body}
    </details>`;
  groups.forEach(g => renderList(`hist-grp-${g.label}`, g.rows, 'gray'));   // 重用既有清單列（含星號/漲跌幅）
}

// ── P2-24 星號快速加自選 ─────────────────────────────────────────
// 清單股的 id/name/mkt 現成（stocks.json／history／watchlist 都有）→ **直接寫 watchlist_v1、不發驗證請求**
// （與手動輸入股號的 watchlistAdd 不同，那條才需要 _resolveStock 查中文名/市場）。
function _starHtml(id) {
  const on = getWatch().some(x => x.id === id);
  return `<span onclick="event.stopPropagation();watchStarToggle('${id}')" role="button" tabindex="0"
    aria-label="${on ? '移出自選' : '加入自選'}" title="${on ? '移出自選' : '加入自選'}"
    class="px-1 select-none cursor-pointer ${on ? 'text-yellow-400' : 'text-gray-600 hover:text-yellow-300'}"
    data-star="${id}">${on ? '★' : '☆'}</span>`;
}
function watchStarToggle(id) {
  const a = getWatch();
  const i = a.findIndex(x => x.id === id);
  if (i >= 0) {
    a.splice(i, 1); setWatch(a);
  } else {
    if (a.length >= WATCH_MAX) { alert(`自選股已達 ${WATCH_MAX} 支上限，請先移除`); return; }
    if (!localStorage.getItem(WATCH_NOTICE)) showWatchNotice();
    const m = (window.STOCKS_BY_ID || {})[id] || {};
    a.push({ id, name: m.name || id, mkt: m.mkt });      // 只存三鍵，與手動新增同 schema
    setWatch(a);
  }
  _refreshStars();
  renderWatchlist();
...
// 三份清單＋折疊區塊的同名星號一起更新（不重繪整表，避免捲動位置跳動）
function _refreshStars() {
  const on = new Set(getWatch().map(x => x.id));
  document.querySelectorAll('[data-star]').forEach(el => {
    const id = el.getAttribute('data-star'), lit = on.has(id);
    el.textContent = lit ? '★' : '☆';
    el.className = `px-1 select-none cursor-pointer ${lit ? 'text-yellow-400' : 'text-gray-600 hover:text-yellow-300'}`;
    el.setAttribute('aria-label', lit ? '移出自選' : '加入自選');
    el.setAttribute('title', lit ? '移出自選' : '加入自選');
  });
}

function renderList(containerId, stocks, color, opts = {}) {
  const container = document.getElementById(containerId);
  if (!container) return;
  container.innerHTML = stocks.map(s => {
    const n = opts.streak ? (opts.streak[s.id] || 0) : 0;   // P2-24：連續 ×N（N≥2 才顯示）
    const streak = n >= 2
      ? `<span class="ml-2 text-[10px] px-1.5 py-0.5 rounded bg-orange-900/60 text-orange-300 align-middle">連續 ×${n}</span>` : '';
    return `
    <div class="w-full bg-gray-800 hover:bg-gray-700 rounded-lg transition flex items-center">
      ${_starHtml(s.id)}
      <button onclick="openModal('${s.id}', '${s.name}')"
        class="flex-1 text-left px-2 py-3 flex items-center justify-between gap-2">
        <span><span class="font-bold text-${color}-400">${s.id}</span><span class="ml-2 text-${color}-400">${s.name}</span>${streak}</span>
...
function _chgHtml(c) {
  ...
  // P2-24（Ball 指定）：本日% 維持 red-400/green-400；**本週% 淺一階**（red-300 #fca5a5／green-300 #86efac），
  // 且顏色跟**該欄自身**的正負走——「本日跌＋本週漲」會是綠日%＋淺紅週%，兩欄各自表述不互相牽動。
  const seg = (label, v, light) => v == null ? '' :
    `<span class="${v > 0 ? (light ? 'text-red-300' : 'text-red-400')
                  : v < 0 ? (light ? 'text-green-300' : 'text-green-400') : 'text-gray-400'}">${label} ${v >= 0 ? '+' : ''}${v.toFixed(1)}%</span>`;
  const parts = [seg('本日', c.chgDay, false), seg('本週', c.chgWeek, true)].filter(Boolean);
...
// 失效演練開關（規格 §6 錨3）：Console 打 `__forceChipFallback = true` 後重開任一彈窗，
// 應見「黃底警示列＋20/60/120 灰化＋當日/5日 由 T86 接手」；改回 false 重開即恢復。
// ⚠ 必須在 ensureFinmindChip 的**快取檢查之前**判斷，否則已快取的股票切開關無效。
// 也可用網址參數觸發（免開 Console，手機亦可測）：`?chipfallback=1`
var __forceChipFallback = new URLSearchParams(location.search).get('chipfallback') === '1';
...
async function ensureFinmindChip(stockId) {
  ...
  // 失效演練開關要在**快取之前**判斷：否則同股已快取成功時，切開關不會生效（2026-07-19 實測踩到）
  if (typeof __forceChipFallback !== 'undefined' && __forceChipFallback) { renderChipWarn(false); return null; }
...
// P2-26 Part B：SP（賣Put）訊號＝sp(t)>sp(t-1) 且 sp_amt(t)>sp_amt(t-1)（單日相鄰對比）。
// 🔴 結算日防呆：結算日當日與其次一交易日不觸發（未平倉重建期機械跳動非真訊號）。
// ⚠ 評分接線（+1）未做：驗證顯示訊號無條件邊際與市場基準幾乎重疊、不支持有可操作預測力，
// 本區塊只做顯示不接評分。
function _spSignalDates(hist, settleDates) {
  const settleSet = new Set(settleDates || []);
  // 結算日次一交易日：用 hist 本身的日期序列找每個結算日的下一筆
  const blocked = new Set(settleSet);
  hist.forEach((r, i) => {
    if (settleSet.has(r.date) && i + 1 < hist.length) blocked.add(hist[i + 1].date);
  });
  const fired = new Set();
  hist.forEach((r, i) => {
    if (i === 0 || blocked.has(r.date)) return;
    const prev = hist[i - 1];
    if (r.sp == null || r.sp_amt == null || prev.sp == null || prev.sp_amt == null) return;  // 舊列缺 sp_amt 不標
    if (r.sp > prev.sp && r.sp_amt > prev.sp_amt) fired.add(r.date);
  });
  return fired;
}
...
function openOptionModal() {
  loadTaifexJson().then(data => {
    const hist = data?.settlement_history;
    if (!hist?.length) return;
    const spFired = _spSignalDates(hist, data.recent_settlement_dates);
...
const IDX_TARGETS = [
  ...
  { name: "日圓兌美元", t: "y", sym: "JPYUSD=X" },   // 1日圓=多少美元；貼近年高＝日圓真的變強，色階方向正確，不需中性灰
...
function renderWatchlist() {
  ...
  box.innerHTML = a.map(s => `
    <div class="flex items-center bg-gray-800 hover:bg-gray-700 rounded-lg transition">
      ${_starHtml(s.id)}
      <button onclick="openModal('${s.id}','${s.name}')" class="flex-1 text-left px-2 py-3 flex items-center justify-between gap-2">
