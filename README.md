...
### 台指期貨（futures）
- 外資台指期淨口數（TX + MTX/4）
- 散戶期貨淨部位 = −(自營＋投信＋外資 未平倉淨額)：小台（MTX）＋微台（TMF ÷5）合計，以偏多/偏空標示
  - 來源頁每機構 6 數字 [交易買/賣/淨, 未平倉買/賣/淨]，淨OI索引：自營=5、投信=11、外資=17（index 40 為合計，程式有驗算）
...
| 項目 | 訊號分 | 權重 |
|------|--------|------|
| 外資現貨買賣超 | ±2（≥30億／≤−30億）、±1（≥5億／≤−5億） | ×2 |
| 外資期貨淨部位（TX+MTX/4） | ±2（≥2000口）、±1（≥500口） | ×2 |
| 結算比（壓力指標，高=偏空） | −2（≥5000）、−1（≥2000）、+1（≤500） | ×2 |
| P/C Ratio | −1（>1.5）、+1（<0.7） | ×1 |
| 選擇權方向（Call/Put 淨部位） | ±1 | ×1 |
| 投信買賣超 | ±1（≥5億／≤−5億） | ×1 |

> 總分 ≥+5 強烈偏多 / ≥+2 偏多 / ≤−5 強烈偏空 / ≤−2 偏空 / 其餘中性‧分歧
> 另有現貨/期貨方向分歧警示（如外資現貨大買但期貨大空）
