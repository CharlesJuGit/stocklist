...
連結為**從未替換的佔位符** `https://buymeacoffee.com/YOUR_USERNAME`，
使用者點擊導向錯誤頁，**比沒有按鈕更糟**，自 2026-08-16 立案起持續在線。
移除處留註解說明原因。**cache-buster `v=20260816a` → `v=20260828a`**（既有規則）。
⚠ 收款平台仍待 Ball 選定，本案未結。

**🔴 Fix（P2-16 驗收條件在機制上驗不到）：** `fetch_taifex.py` 的
`update_log = update_log[-12:]` → **`[-40:]`**。
- 前：保留 12 筆，一天約 6 班 → **僅約 2 天**
- 後：保留 40 筆 → **約一週**
根因：P2-16 的殘驗收①要看「**週五** 16:47 班的落點」，
**12 筆的保留期跨不過一週，該條件永遠驗不到**——實測 `update_log` 最早僅到
2026-08-21 17:07，若確有 16:47 那班早被裁掉。
📌 **症狀本身已解**：週五 08-21 17:07 該筆 `inst=2026-08-21`，三大法人週五下午確有更新。
📌 殘驗收②（手動鈕）待 Ball 按一次手動更新鈕即可驗收（12 筆全為 `schedule`，從未有手動）。

**✅ 結案 P2-26殘**：05-14~05-21 的 6 列壞 sp 已自然汰除——
`settlement_history` 最早僅到 2026-05-29；**病徵列（bc/sc 為 None 但 sp/bp 有值）0 筆**；
`sp` 範圍 3825~23783，無「5萬多」壞值。案卡預估「約 11 交易日汰除」成立且三個月無復發。
⚠ 復發條件登記備查：日後若從來源回填歷史，同型態舊列可能再現，屆時再套原定修法。

**✅ 結案 P2-23**：本體已於 `083190d` 驗收（第二層 fallback 自癒恢復），
殘觀察項案卡明文「非驗收條件」，轉為註記不再掛案。
